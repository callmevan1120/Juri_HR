...
| 7 | The `legacy-reference` branch still carries the old `Vercel Production` and `Deploy` workflows, which fail on every push to that branch | Cosmetic only — red runs in the Actions tab, no effect on `main` or on CI | Leave the branch untouched (it is the visual reference); disable those two workflows in the repository's Actions settings if the noise matters |
