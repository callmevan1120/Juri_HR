...
- `[~]` implemented, but still needs production UAT/sign-off or policy decision.
- `[ ]` production gate not completed by code.
...
- `[~]` Final dashboard/report parity must be signed off against the exact final frozen dump.
...
- `[ ]` Operator training is not complete.
- `[ ]` Owner/admin production sign-off is not complete.
- `[~]` toko-pandan archive/read-only/redirect policy still needs final owner decision.
...
- `[~]` Owner/admin parity sign-off against final frozen dump.
...
- `[ ]` Cashier operator UAT/sign-off.
- `[~]` Final printed receipt layout sign-off.
...
- `[ ]` Purchasing operator UAT/sign-off.
...
- `[~]` Owner decision on whether destructive hard-delete is still allowed.
...
- `[~]` Finance UAT for legacy expense type naming and grouping.
...
- `[~]` Approval policy for stock adjustment.
- `[ ]` Warehouse operator UAT/sign-off.
...
- `[~]` Owner decision on final PDF/XLSX scope.
- `[ ]` Owner/admin report sign-off.
...
- `[ ]` Production user invite and RBAC sign-off.
...
- `[~]` Final parity must use the exact final frozen dump, not screenshots from a different timestamp.
...
- `[ ]` Operators trained on new menu.
- `[ ]` Cashier/purchasing/warehouse/finance UAT completed.
- `[ ]` Owner/admin signs off.
- `[~]` toko-pandan archive/read-only/redirect policy selected.
...
- `[ ]` UAT evidence collected from cashier lead.
- `[ ]` UAT evidence collected from purchasing/admin gudang.
- `[ ]` UAT evidence collected from warehouse/admin gudang.
- `[ ]` UAT evidence collected from finance/admin.
- `[ ]` UAT evidence collected from owner/admin.
...
- `[ ]` Cashier training attendance collected.
- `[ ]` Purchasing/admin gudang training attendance collected.
- `[ ]` Warehouse/admin gudang training attendance collected.
- `[ ]` Finance/admin training attendance collected.
- `[ ]` Owner/admin training attendance collected.
- `[ ]` Head admin training attendance collected.
...
- `[ ]` Cashier lead signs POS flow.
- `[ ]` Purchasing/admin gudang signs purchasing flow.
- `[ ]` Warehouse/admin gudang signs inventory flow.
- `[ ]` Finance/admin signs finance and Accounting Workspace contribution.
- `[ ]` Owner/admin signs dashboard/report totals and reconciliation.
- `[ ]` Head/admin signs production RBAC and branch/store assignment.
- `[ ]` Owner chooses toko-pandan retirement mode.
- `[ ]` Owner chooses hard-delete policy.
- `[ ]` Owner plus warehouse chooses stock adjustment approval policy.
- `[ ]` Owner chooses final report export scope.
- `[ ]` Head/admin chooses whether migration workspace remains enabled for head only or disabled with `toko_pos.migration_enabled=false`.
