...
describe('api client', () => {
                ...
                jsonResponse(
                    {
                        ...
                        _server_messages: JSON.stringify([
                            JSON.stringify({ message: 'Di luar radius' }),
                        ]),
                    },
                    417,
                ),
    ...
    it('clears the session and notifies the handler on 401', async () => {
        ...
        vi.stubGlobal(
            'fetch',
            vi.fn().mockResolvedValue(jsonResponse({ message: 'Not permitted' }, 401)),
        )
    ...
    it('derives the fixture role from the email used to log in', async () => {
        const { request } = await loadClient({ VITE_USE_FIXTURES: 'true' })

        await request('/api/method/login', { method: 'POST', body: { usr: 'hrd@example.com' } })
        const asHrd = await request<{ message: { roles: string[] } }>(
            '/api/method/juri_hr.auth.session',
        )
        expect(asHrd.message.roles).toContain('HR Manager')

        await request('/api/method/login', { method: 'POST', body: { usr: 'budi@example.com' } })
        const asEmployee = await request<{ message: { roles: string[] } }>(
            '/api/method/juri_hr.auth.session',
        )
        expect(asEmployee.message.roles).toEqual(['Employee'])
    })
})
